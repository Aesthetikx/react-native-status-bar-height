...
describe("getStatusBarHeight", () => {
  describe("iOS", () => {
    ...
    test("Should return 47 when device is iOS and have iPhone 12 width and height", () => {
      mockCanvasSize(390, 844);
      getStatusBarHeight = require("./").getStatusBarHeight;
      const expectedSize = 47;
      expect(getStatusBarHeight()).toBe(expectedSize);
    });
    test("Should return 47 when device is iOS and have iPhone 12 Max width and height", () => {
      mockCanvasSize(428, 926);
      getStatusBarHeight = require("./").getStatusBarHeight;
      const expectedSize = 47;
      expect(getStatusBarHeight()).toBe(expectedSize);
    });
    test("Should return 47 when device is iOS and have iPhone 12 Mini width and height", () => {
      mockCanvasSize(375, 812);
      getStatusBarHeight = require("./").getStatusBarHeight;
      const expectedSize = 47;
      expect(getStatusBarHeight()).toBe(expectedSize);
